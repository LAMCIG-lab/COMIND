...
COMIND_ROOT = Path("/home/dsemchin/COMIND")  # sibling of COMIND_experiments
if COMIND_ROOT.is_dir():
    sys.path.insert(0, str(COMIND_ROOT))

from COMIND_transformer.subtyping_em_transformer import SubtypingEM
from COMIND_transformer.utils import initialize_f_eigen, fit_mixedlm_beta_from_clinical
...
### INITIALIZE F - single fixed f_init from eigen init
...
# Tie lambda_kappa to lambda_f for each grid candidate (no extra grid dimension)
...
def build_estimator(params, K, f_init, t_max, rng_seed=75):
    ...
    # Backward compatible with older SubtypingEM signatures.
    if "lambda_kappa" in inspect.signature(SubtypingEM.__init__).parameters:
        est_kwargs["lambda_kappa"] = params["lambda_kappa"]
    return SubtypingEM(**est_kwargs)
