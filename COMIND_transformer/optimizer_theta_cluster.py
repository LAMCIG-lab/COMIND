...
def theta_cluster_loss(params: np.ndarray, t_obs: np.ndarray, x_obs: np.ndarray,
                       ...
                       lambda_f: float, kappa: np.ndarray = None, obs_weights: np.ndarray = None) -> float:
    """
    ...
    if obs_weights is None:
        obs_weights = np.ones(x_obs.shape[0]) # (n_obs,)
    ...
    loss = np.sum(obs_weights[:, None] * residuals**2) + lambda_f * np.sum(np.abs(f))
    return loss

def theta_cluster_loss_jac(params: np.ndarray, t_obs: np.ndarray, x_obs: np.ndarray,
                           K: np.ndarray, t_span: np.ndarray, s: np.ndarray, scalar_K: float,
                           lambda_f: float, kappa: np.ndarray = None, obs_weights: np.ndarray = None) -> tuple:
    """
    ...
    grad_f = -2 * np.sum(obs_weights[:, None] * residuals * (df_obs * s[None, :]), axis=0) + lambda_f * np.sign(f)
...
def fit_theta_cluster(X_obs: np.ndarray, dt_obs: np.ndarray, ids: np.ndarray, K: np.ndarray,
                      ...
                      obs_weights: np.ndarray = None) -> np.ndarray:
    """
    ...
    args = (t_pred, X_obs, K, t_span, s, scalar_K, lambda_f, kappa, obs_weights)
