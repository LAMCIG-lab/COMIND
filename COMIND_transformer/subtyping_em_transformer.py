...
from .optimizer_cognitive_regression import fit_linear_cog_regression_multi, fit_linear_cog_regression_weighted
...
class SubtypingEM(BaseEstimator, TransformerMixin):
    ...
    After exhausting θ stages for an outer iteration, if the trial LSE does not improve
    enough vs. ``best_lse`` (``epsilon`` / ``relative_tolerance``), a patience counter
    increments. An improvement above that threshold resets patience. When patience
    reaches ``patience`` consecutive sub-threshold outer iterations, EM exits and
    restores the checkpoint with the lowest LSE seen so far.
    ...
    def fit(self, X: np.ndarray, y: np.ndarray = None):
        ...
        if self.jac_toggle:
            solver_phases = ["lbfgs_exact", "nelder_mead"]
        ...
        responsibilities = np.full((n_patients, self.n_subtypes), 1.0 / self.n_subtypes) # uniform initial responsibilities
        ...
        def _snapshot_em_state():
            return {
                "s": np.copy(current_s),
                "kappa": np.asarray(current_kappa, dtype=float).copy(),
                "scalar_K": float(current_scalar_K),
                "cluster_f": [np.ravel(fc).copy() for fc in cluster_f],
                "beta": np.copy(current_beta),
                "assignments": np.copy(assignments),
                "responsibilities": np.copy(responsibilities),
                "cog_a": [np.copy(a) for a in cluster_cog_a],
                "cog_b": [float(b) for b in cluster_cog_b],
            }

        def _restore_em_state(snap):
            nonlocal current_s, current_kappa, current_scalar_K, cluster_f
            nonlocal current_beta, assignments, responsibilities
            nonlocal cluster_cog_a, cluster_cog_b
            current_s = np.copy(snap["s"])
            current_kappa = np.copy(snap["kappa"])
            current_scalar_K = snap["scalar_K"]
            cluster_f = [fc.copy() for fc in snap["cluster_f"]]
            current_beta = np.copy(snap["beta"])
            assignments = np.copy(snap["assignments"])
            responsibilities = np.copy(snap["responsibilities"])
            cluster_cog_a = [np.copy(a) for a in snap["cog_a"]]
            cluster_cog_b = list(snap["cog_b"])

        best_state = _snapshot_em_state()
        ...
        while loop_iter < self.max_iter:
            ...
            ck_responsibilities = np.copy(responsibilities)
            ...
            while True:
                ...
                responsibilities = np.copy(ck_responsibilities)
                ...
                for subtype in range(self.n_subtypes):
                    ...
                    cluster_cog_a[subtype], cluster_cog_b[subtype] = fit_linear_cog_regression_weighted(
                        cog, dt, current_beta, ids,
                        weights=responsibilities[:, subtype]
                    )

                ## STEP 3: CLUSTER LEVEL --> update f[subtype] for each cluster (scalar_K is now global)
                for subtype in range(self.n_subtypes):
                    obs_weights = responsibilities[ids, subtype]
                    ...
                    f_cluster = fit_theta_cluster(
                        ...
                        solver_stage=current_solver,
                        beta_pred=current_beta,
                        f_guess=np.ravel(cluster_f[subtype]),
                        rng=rng,
                        kappa=current_kappa,
            ...
            # ``acceptable`` set by inner θ-solver loop (same threshold vs. pre-iter best_lse)
            if np.isfinite(lse) and lse < best_lse:
                best_lse = float(lse)
                best_state = _snapshot_em_state()
            ...
            if acceptable:
                ...
                patience_streak += 1
                if patience_streak >= self.patience:
                    if self.verbose >= 1:
                        print(
                            f"EM: {patience_streak} consecutive outer iteration(s) without "
                            f"LSE improvement above threshold (epsilon={self.epsilon}, "
                            f"relative_tolerance={self.relative_tolerance}); "
                            f"best LSE={best_lse:.6g}, last trial LSE={lse:.6g}. "
                            f"Exiting early at outer iter {loop_iter}."
                        )
                    _restore_em_state(best_state)
                    lse = float(best_lse)
                    break
        ...
        self.final_responsibilities = responsibilities
        ...
        if X is not None and len(X) > 0 and "subtype_true" in X[0]:
            ...
            if np.all(true_labels >= 0):  # Only match if all have valid true labels
                try:
                    from .utils import get_subtype_mapping
                    self.subtype_mapping = get_subtype_mapping(assignments, true_labels)
                    self.final_assignments_matched = self.subtype_mapping[assignments]
    ...
    def _compute_responsibilities(self, X_obs, dt, ids, cog, beta, cluster_f, scalar_K, kappa, s,
                                  K, t_span, cluster_cog_a, cluster_cog_b, lambda_cog, temperature = 1.0):
        """
        Compute responsibilities for each patient for each cluster.
        """
        unique_ids = np.unique(ids)
        n_patients = len(unique_ids)
        n_subtypes = len(cluster_f)
        n_biomarkers = X_obs.shape[1]
        responsibilities = np.zeros((n_patients, n_subtypes))
        ...
        for idx, patient_id in enumerate(unique_ids):
            ...
            for subtype in range(n_subtypes):
                ...
                if lambda_cog > 0:
                    ...
                    sse_vec[subtype] += lambda_cog * np.sum((dt_i + beta_i - cog_pred) ** 2)

            # log_r = -sse_vec / temperature
            log_r = -sse_vec / (temperature * (np.mean(sse_vec) + 1e-12))
            log_r -= np.max(log_r)
            r = np.exp(log_r) # softmax numerator
            r /= r.sum() # softmax denominator

            responsibilities[idx, :] = r
    ...
    def _update_assignments(self, X_obs, dt, ids, cog, beta, cluster_f, scalar_K, kappa, s,
        ...
        """
        Update cluster assignments using soft assignment based on responsibilities.
        """
        r = self._compute_responsibilities(X_obs, dt, ids, cog, beta, cluster_f, scalar_K, kappa, s,
                                          K, t_span, cluster_cog_a, cluster_cog_b, lambda_cog)
    ...
    def compute_subtype_mapping(
        self,
        true_f_list=None,
        true_assignments=None,
        fitted_assignments=None,
        method: str = "auto",
        verbose: bool = True,
    ):
        """
        Compute mapping from fitted subtype index -> true subtype index.

        For classification metrics (confusion matrix, ARI), prefer assignment overlap
        when ``true_assignments`` is available. F-based matching is only reliable when
        ``cluster_f`` is well separated; collapsed ``f`` estimates can permute labels
        even when patient assignments are correct.

        Parameters
        ----------
        true_f_list : Sequence[np.ndarray], optional
            True f vectors per subtype (required for ``method='f'`` or ``'auto'`` without
            assignments).
        true_assignments : array-like, optional
            True patient subtype labels, same order as training patients.
        fitted_assignments : array-like, optional
            Fitted labels to match; defaults to ``final_assignments``.
        method : {'auto', 'assignments', 'f'}
            ``'assignments'``: Hungarian on patient overlap (best for CM/ARI).
            ``'f'``: Hungarian on ||f_fitted - f_true||.
            ``'auto'``: assignments if ``true_assignments`` is given, else f.
        verbose : bool
            Whether to print the mapping.
        """
        from .utils import get_subtype_mapping, get_subtype_mapping_from_f

        if method not in ("auto", "assignments", "f"):
            raise ValueError(f"method must be 'auto', 'assignments', or 'f', got {method!r}")

        use_assignments = method == "assignments" or (
            method == "auto" and true_assignments is not None
        )

        if use_assignments:
            if true_assignments is None:
                raise ValueError("true_assignments is required for method='assignments'")
            if fitted_assignments is None:
                if self.final_assignments is None:
                    raise ValueError(
                        "fitted_assignments must be provided when final_assignments "
                        "is not set on the model"
                    )
                fitted_assignments = self.final_assignments
            self.subtype_mapping = get_subtype_mapping(
                np.asarray(fitted_assignments), np.asarray(true_assignments)
            )
            if verbose:
                print(
                    "\nSubtype mapping (fitted -> true), from assignment overlap:"
                )
        else:
            if true_f_list is None:
                raise ValueError(
                    "true_f_list is required for method='f' when true_assignments "
                    "is not provided"
                )
            self.subtype_mapping = get_subtype_mapping_from_f(
                self.cluster_f, true_f_list
            )
            if verbose:
                print("\nSubtype mapping (fitted -> true), from f parameters:")

        if verbose:
            print(f"  mapping array: {self.subtype_mapping}")
            for fitted_subtype in range(len(self.subtype_mapping)):
                print(
                    f"  Fitted subtype {fitted_subtype} -> "
                    f"True subtype {self.subtype_mapping[fitted_subtype]}"
                )
    ...
    def transform(self, X: list[dict], use_cognitive_prior: bool = True) -> np.ndarray:
        ...
        for idx, p in enumerate(tqdm(X, desc="Estimating beta and subtype assignments")):
            ...
            beta_i_vec, _ = estimate_beta(
                ...
                s=self.final_s,
                responsibilities=np.array([[1.0]], dtype=float),
