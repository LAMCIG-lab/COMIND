...
def _vectorized_beta_loss_and_grad(
    ...
    n_subtypes = responsibilities.shape[1]
    total_loss = 0.0
    ...
    for idx in range(n_patients):
        ...
        for subtype in range(n_subtypes):
            weight = responsibilities[idx, subtype]
            if weight < 1e-8:
                continue
            ...
            residuals = X_obs_i.T - X_interp_i
            total_loss += weight * np.sum(residuals ** 2)
            gradient[idx] += weight * (-2.0 * np.sum(residuals * np.array(
            ...
            if lambda_cog > 0:
                ...
                total_loss += weight * lambda_cog * np.sum((t_pred_i - cog_pred) ** 2)
                gradient[idx] += weight * 2.0 * lambda_cog * np.sum(t_pred_i - cog_pred)
...
def estimate_beta(
    ...
    if responsibilities is None:
        ...
        responsibilities = np.ones((n_patients, n_subtypes)) / n_subtypes
    ...
    def loss_and_grad_func(beta_vec):
        return _vectorized_beta_loss_and_grad(
            beta_vec, X_obs, dt, ids, cog, t_span, cluster_f, scalar_K, s,
            responsibilities, K, cluster_cog_a, cluster_cog_b,
            lambda_cog, lambda_jsd, t_max, X_pred_by_cluster, dxdt_by_cluster,
            jsd_n_bins, jsd_bandwidth, jsd_value_range, lambda_beta, beta_mean, beta_var,
        )
